...
export default function Contact() {
  ...
  // 스크롤 위치 조정 (Contact 섹션으로 스크롤)
  useEffect(() => {
    // URL에 contact 해시가 있으면 해당 섹션으로 스크롤
    const handleScroll = () => {
      if (window.location.hash === '#contact') {
        const contactSection = document.getElementById('contact');
        if (contactSection) {
          // 약간의 지연 후 스크롤 (컴포넌트 렌더링 완료 보장)
          setTimeout(() => {
